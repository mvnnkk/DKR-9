...
begin
  ...
  if (fEdit.eName.Text= '') or (fEdit.epochta.Text= '') then exit;
  ...
  SG.Cells[1, SG.RowCount-1]:= fEdit.epochta.Text;
...
begin
  ...
  SG.Cells[0, 0]:= 'Имя';
  SG.Cells[1, 0]:= 'Почта';
  SG.Cells[2, 0]:= 'Предмет';
  SG.ColWidths[0]:= 365;
  SG.ColWidths[1]:= 230;
  SG.ColWidths[2]:= 170;
  //если файла данных нет, просто выходим:
  if not FileExists(adres + 'pochtes.dat') then exit;
  ...
  try
    ...
    while not Eof(f) do begin
      ...
      //добавляем в сетку новую строку, и заполняем её:
        ...
        SG.Cells[1, SG.RowCount-1]:= MyCont.pochta;
...
begin
  ...
  fEdit.epochta.Text:= SG.Cells[1, SG.Row];
  ...
  if fEdit.ModalResult = mrOk then begin
    ...
    SG.Cells[1, SG.Row]:= fEdit.epochta.Text;
